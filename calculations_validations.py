def sort_by_last_value_in_each_el(lst: list[list[float]]) -> list[list[float]]:   # 2d list/ matrix
    n = len(lst)
    for i in range(n):
...
def calculate_distance_between_two_points(ra_dec1: tuple, ra_dec2: tuple) -> float:
    ra1, dec1 = ra_dec1
    ra2, dec2 = ra_dec2
...
def sort_n_stars_by_brightness(lst: list, number_of_stars: int) -> list:
    sorted_by_brightness = sort_by_last_value_in_each_el(lst)
    return sorted_by_brightness[:number_of_stars]


def is_in_fov(star_ra: float, star_dec: float, given_ra: float, given_dec: float, fov_h: float, fov_v: float):
    h_min = given_ra - fov_h/2
    h_max = given_ra + fov_h/2
    v_min = given_dec - fov_v/2
    v_max = given_dec + fov_v/2
    if not (is_valid_points(h_min, v_min) and is_valid_points(h_max, v_max)):
